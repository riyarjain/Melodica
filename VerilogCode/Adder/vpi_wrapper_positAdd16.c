 ...
 *     #include "vpi_wrapper_positAdd16.h"
 *     void (*vlog_startup_routines[])() = { positAdd16_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_positAdd16 call=positAdd16_calltf size=16 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_positAdd16 vpiSysFuncSized 16 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
PLI_INT32 positAdd16_calltf(PLI_BYTE8 *user_data)
{
  ...
  vpi_result = positAdd16(arg_1, arg_2);
...
/* sft: $imported_positAdd16 vpiSysFuncSized 16 unsigned */

/* tab: $imported_positAdd16 call=positAdd16_calltf size=16 acc=rw:%TASK */

PLI_INT32 positAdd16_sizetf(PLI_BYTE8 *user_data)
{
  return 16u;
}
...
void positAdd16_vpi_register()
{
  ...
  tf_data.tfname = "$imported_positAdd16";
  tf_data.calltf = positAdd16_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = positAdd16_sizetf;
  tf_data.user_data = "$imported_positAdd16";
